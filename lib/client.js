...
function Client(opts) {
  ...
  function useOAuthToken (oauthToken, cb) {
    if (!oauthToken || typeof oauthToken !== 'string') {
      return cb(new Error("You must provide a oauth token."));
    }
  ...
  var owners = {
    get: function(options, cb) {
      ...
      sendRequest({
        method: 'GET',
        url: self.baseUrl + '/owners/v2/owners/',
        qs: options
      }, cb);
    }
  }
  ...
  function sendRequest (call, cb) {
    if (!self.key && !self.token && !self.oauthToken) {
    ...
    } else if (self.oauthToken) {
      call.headers = {'Authorization': 'Bearer ' + self.oauthToken};
    }
